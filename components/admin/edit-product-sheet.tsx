...
export function EditProductSheet({ product }: EditSheetProps) {
    ...
    const close = () => router.push("/ame-studio-ops/inventory");
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
            <div onClick={attemptClose} className="absolute inset-0 bg-black/20 backdrop-blur-sm transition-opacity cursor-pointer" />

            <div className="relative w-full max-w-md bg-white h-full shadow-2xl overflow-hidden border-l border-gray-100 flex flex-col animate-in slide-in-from-right duration-300">
                ...
                <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10 shrink-0">
                    ...
                    <button type="button" onClick={attemptClose} className="p-2 bg-gray-50 text-gray-400 hover:text-gray-600 rounded-full hover:bg-gray-100 transition-colors">
                        <X size={20} />
                    </button>
                </div>

                <form id="edit-form" onSubmit={handleSubmit} onChange={() => setIsDirty(true)} className="flex-1 overflow-y-auto flex flex-col">
                    <div className="p-6 space-y-6 flex-1">
                        ...
                        <div className="space-y-3">
                            ...
                            {imageError && (
                                <div className="flex items-center gap-2 text-red-600 bg-red-50 px-3 py-2.5 rounded-lg border border-red-100 text-xs font-medium animate-in fade-in slide-in-from-top-1">
                                    <AlertTriangle size={14} className="shrink-0" /><p>{imageError}</p>
                                </div>
                            )}
                        </div>

                        <div><label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Nombre</label><input name="name" type="text" defaultValue={product.name} required className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-green-500" /></div>

                        <div>
                            <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Categoría</label>
                            <select name="category" value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm outline-none bg-white focus:border-green-500">
                                <option value="READY_MADE">Prenda de Vestir</option><option value="SERVICE">Servicio</option>
                            </select>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="gender" defaultValue={product.gender || ""} className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm bg-white outline-none focus:border-green-500">
                                    <option value="">Sin especificar</option>{sortedGenders.map(g => (<option key={g} value={g}>{formatLabel(g)}</option>))}
                                </select>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Tipo Prenda</label>
                                <select name="clothingType" defaultValue={product.clothingType || ""} className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm bg-white outline-none focus:border-green-500">
                                    <option value="">Sin especificar</option>{sortedClothingTypes.map(c => (<option key={c} value={c}>{formatLabel(c)}</option>))}
                                </select>
                            </div>
                        </div>
                        ...
                        {category !== 'SERVICE' && (
                            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <button type="button" onClick={addSize} className="text-xs font-medium text-green-600 hover:text-green-700 flex items-center gap-1 bg-green-50 px-2 py-1 rounded-md"><Plus size={14} /> Agregar Talla</button>
                                </div>
                                {sizes.length > 0 ? (
                                    <div className="space-y-2 mb-3">
                                        {sizes.map((s, index) => (
                                            <div key={index} className="flex items-center gap-2">
                                                <input type="text" placeholder="Ej: S, 32, XL" value={s.size} onChange={(e) => updateSize(index, 'size', e.target.value)} className="w-1/2 px-3 py-1.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-green-500" required />
                                                <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="0" value={s.stock} onChange={(e) => { let val = e.target.value.replace(/\D/g, ''); val = val.replace(/^0+(?=\d)/, ''); updateSize(index, 'stock', val); }} className="w-1/3 px-3 py-1.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-green-500" required />
                                                <button type="button" onClick={() => removeSize(index)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                                            </div>
                                        ))}
                                    </div>
                                ) : ( <p className="text-xs text-gray-400 italic mb-3">No hay tallas configuradas.</p> )}
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div className="relative"><span className="absolute left-3 top-2 text-gray-400 text-sm">S/</span><input name="price" type="number" step="0.01" defaultValue={product.price} required className="w-full pl-8 pr-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-green-500" /></div>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wide flex justify-between">Stock Total {hasSizes && <span className="text-green-600 normal-case">(Auto)</span>}</label>
                                <input name="stock" type="number" required value={hasSizes ? totalSizesStock : manualStock} onChange={(e) => !hasSizes && setManualStock(e.target.value)} readOnly={hasSizes} placeholder="0" className={`w-full px-4 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-green-500 ${hasSizes ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white'}`} />
                            </div>
                        </div>

                        <div><label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Descripción</label><textarea name="description" rows={4} defaultValue={product.description || ""} className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-green-500 resize-none" /></div>
                    </div>
                    ...
                    <div className="p-4 border-t border-gray-100 bg-white flex justify-end gap-3 shrink-0">
                        ...
                        <button type="submit" disabled={isLoading} className="flex items-center gap-2 px-6 py-2 text-sm font-bold text-white bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:animate-pulse rounded-lg transition-colors shadow-sm">
