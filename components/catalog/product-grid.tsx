    ...
    const productCards = displayProducts.map((product) => {
        ...
        return (
            <div key={product.id} className={itemClasses} tabIndex={0}>
                ...
                {/* 1. CONTENEDOR DE IMAGEN CON BADGES EN LA PARTE INFERIOR */}
                <div className="relative overflow-hidden rounded-xl">
                    <ProductCardGallery product={productToDisplay} isOutOfStock={isOutOfStock} />

                    {/* 👇 BADGES EN LA BASE DE LA IMAGEN */}
                    <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/60 to-transparent z-10 flex gap-1 pointer-events-none">
                        {product.gender && (
                            <Badge className="bg-white/90 backdrop-blur-sm text-blue-700 border-none text-[9px] font-bold py-0 px-1.5 shadow-sm">
                                {formatLabel(product.gender)}
                            </Badge>
                        )}
                        {product.clothingType && (
                            <Badge className="bg-white/90 backdrop-blur-sm text-purple-700 border-none text-[9px] font-bold py-0 px-1.5 shadow-sm">
                                {formatLabel(product.clothingType)}
                            </Badge>
                        )}
                        {isService && (
                            <Badge className="bg-green-500 text-white border-none text-[9px] font-bold py-0 px-1.5 shadow-sm">
                                SERVICIO
                            </Badge>
                        )}
                    </div>
                ...
                {/* 2. INFORMACIÓN DEL PRODUCTO (Alineación perfecta) */}
                <div className="flex flex-col flex-1 mt-3">
                    <div className="flex flex-col mb-3">
                        <Link href={`/product/${product.id}`} className="group/link block">
                            {/* Truncamos a una sola línea para orden visual absoluto */}
                            <h3 className="text-base font-semibold text-gray-900 truncate group-hover/link:text-green-600 transition-colors" title={product.name}>
                                {product.name}
                            </h3>
                        </Link>
                    ...
                    <div className="mt-auto flex flex-col gap-4">
                        ...
                        <div className="w-full">
                            {isOutOfStock ? (
                                ...
                                <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="w-full h-12 flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20bd5a] text-white font-medium rounded-lg text-sm transition-colors shadow-sm active:scale-[0.98]">
                                ...
                                <div className="grid grid-cols-[1fr_48px] gap-2 h-12">
                                    ...
                                    <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="flex h-full w-full items-center justify-center rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white shadow-md active:scale-[0.98]">
